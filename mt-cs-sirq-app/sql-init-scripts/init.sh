...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    -------------------------------------------------------------------------------------------------
    ----------------------------------- job_type TYPE -------------------------------------------
    CREATE TYPE job_type AS ENUM ('AUCTIONS_SETTLING', 'SPOT_ASSIGNMENT', 'FLOW_ENFORCER');
    -------------------------------------------------------------------------------------------------
    ----------------------------------- jobs TABLE ------------------------------------------
    ...
    CREATE OR REPLACE VIEW masterthesis_schema.v_loose_flows AS (
            WITH never_checked_in_at_charger AS (
                ...
                WHERE charger_checkin_ts IS NULL
                    AND CURRENT_TIMESTAMP > (spot_assignment_ts + INTERVAL '${MAXIMUM_CHECKIN_SLACK} seconds')
                    AND departure_ts IS NULL
            ),
            never_started_charging AS (
                ...
                WHERE start_charge_ts IS NULL
                    AND CURRENT_TIMESTAMP > (charger_checkin_ts + INTERVAL '${MAXIMUM_START_CHARGING_SLACK} seconds')
                    AND departure_ts IS NULL
            ),
            exceeds_charging_interval AS (
                ...
                WHERE end_charge_ts IS NULL
                    AND CURRENT_TIMESTAMP > (start_charge_ts + INTERVAL '${MAXIMUM_CHARGING_TIME} seconds')
                    AND end_charge_ts IS NULL
            ),
            never_departed AS (
                SELECT *,
                ...
                FROM masterthesis_schema.charging_flows
                WHERE departure_ts IS NULL
                    AND CURRENT_TIMESTAMP > (end_charge_ts + INTERVAL '${MAXIMUM_DEPARTURE_SLACK} seconds')
                    AND departure_ts IS NULL
            )
            SELECT *
