...
export async function handler(
  ...
  log(
    "DEBUG",
    `Charging Flows Middleware entered - company ID retrieved: ${companyId}`,
  );
  ...
  try {
    const resultRows = await sqlConnection.queryObject<{
      ...
      company_name: string;
            ...
            WHERE company_id = ${companyId} AND company_name = 'ChargingStationInfrastructureInc.'
            ;
        `.then((itm) => itm.rows);
    ...
    if (resultRows.length < 1) {
      ...
      return terminateMiddleware(
        "Company ID was either not found or does not have infrastructure rights",
      );
    }
  } catch (err) {
    const thrownError = err as Error;
    log("ERROR", `Charging Flows check error: '${thrownError}'`);
    return new Response(`Internal Server Error: '${thrownError}'`, {
      status: STATUS_CODE.InternalServerError,
    });
  } finally {
    sqlConnection.release();
    log("INFO", "Releasing SQL connection from Charging Flows Middleware");
  }
