...
export const handler: Handlers<Request, ContextState> = {
  async POST(_req, ctx: FreshContext<ContextState>) {
    ...
    log(
      "INFO",
      "/charging-flows/established-sessions/charging-start POST request received",
    );
    ...
    log(
      "DEBUG",
      `Received charging-start POST request for: license plate: '${reqObj.licensePlate}', in charging station with ID: '${reqObj.chargingStationId}, for charger with ID: '${reqObj.chargerId}'`,
    );
    ...
    try {
      ...
      if (openChargingFlowsResult.length > 1) {
        ...
        const setChargingStartResult = await sqlConnection.queryObject<
          {
            sessionId: string;
            chargerId: string;
            chargingStartTimestamp: string;
        ...
        >({
          text: `
                        ...
                        SET start_charge_ts = CURRENT_TIMESTAMP
        ...
        const result: SuccessResponse = {
          sessionId: setChargingStartResult?.[0].sessionId,
          chargerId: setChargingStartResult?.[0].chargerId,
          chargingStartTimestamp: setChargingStartResult?.[0]
            .chargingStartTimestamp,
        };
      ...
      log(
        "INFO",
        "Releasing SQL connection from /api/private/charging-flows/established-sessions/charging-start",
      );
    }
  },
};
