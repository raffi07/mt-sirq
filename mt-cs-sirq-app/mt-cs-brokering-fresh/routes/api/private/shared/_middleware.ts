...
function terminateMiddleware(errorMsg: string) {
  log(
    "ERROR",
    `Shared Middleware error: ${errorMsg}`,
  );
...
export async function handler(
  ...
  log(
    "DEBUG",
    `Shared Middleware entered - company ID retrieved: ${companyId}`,
  );
  ...
  try {
    ...
    if (resultRows.length < 1) {
      log(
        "DEBUG",
        `Shared Middleware: No company found with '${companyId}'`,
      );
      return terminateMiddleware(
    ...
    if (companyName === "TheAdminCompanyInc.") {
      log("INFO", "Shared Middleware: User has admin rights");
    ...
    log("INFO", "Releasing SQL connection from Shared Middleware");
