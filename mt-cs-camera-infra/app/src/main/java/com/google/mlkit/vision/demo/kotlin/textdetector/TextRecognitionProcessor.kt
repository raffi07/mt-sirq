...
class TextRecognitionProcessor(
  ...
  private val textInput: EditText? = textInputField
  private val textDView: TextView? = textDisplay
  ...
  override fun onSuccess(text: Text, graphicOverlay: GraphicOverlay) {
    ...
    if (text.textBlocks.size > 0){
      if (textInput?.text.toString() == "No text detected yet"){
        Log.d(TAG, "Setting the editable text to " + text.text.lines()[0])
        textInput?.setText(text.text.lines()[0])
        Log.d(TAG, "Setting the non-editable text to " + text.text.lines()[0])
        textDView?.text = text.text.lines()[0]
      }
      else {
        Log.d(TAG, "Setting the non-editable text to " + text.text.lines()[0])
        textDView?.text = text.text.lines()[0]
